...
final class TriggerEditorView: NSView, NSTextFieldDelegate {
    ...
    func controlTextDidChange(_ obj: Notification) {
        guard mode.selectedSegment == 1 else { return }
        do {
    ...
    private func groupLabel(_ title: String, x: CGFloat, y: CGFloat, width: CGFloat, path: [Int]) -> NSView {
        let box = NSBox(frame: NSRect(x: x, y: y, width: width, height: 26)); box.boxType = .custom; box.borderType = .lineBorder; box.cornerRadius = 5
        let button = NSButton(title: title, target: self, action: #selector(toggleGroup(_:)))
        button.bezelStyle = .inline; button.frame = NSRect(x: 5, y: 1, width: 54, height: 22); button.identifier = id("toggle", path); box.addSubview(button)
        if !path.isEmpty { let remove = NSButton(title: "−", target: self, action: #selector(removeNode(_:))); remove.bezelStyle = .inline; remove.frame = NSRect(x: width - 30, y: 1, width: 26, height: 22); remove.identifier = id("remove", path); box.addSubview(remove) }
        return box
    }
    ...
    @objc private func conditionChanged(_ sender: NSControl) {
        ...
        if role == "field", let popup = sender as? NSPopUpButton { nextField = popup.titleOfSelectedItem ?? field }
