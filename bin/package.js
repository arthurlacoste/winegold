...
const all = {
  ...
  // start with a number. We're using the version of the underlying library.
...
const darwin = {
  ...
  // The bundle identifier to use in the application's plist (Mac only).
	'app-bundle-id': 'io.winegold.winegold',
  ...
  // The bundle identifier to use in the application helper's plist (Mac only).
	'helper-bundle-id': 'io.winegold.winegold-helper',
...
function buildDarwin(cb) {
	...
	electronPackager(Object.assign({}, all, darwin), (err, buildPath) => {
		...
		infoPlist.CFBundleDocumentTypes = [
			{
				CFBundleTypeExtensions: ['wgml'],
				CFBundleTypeIconFile: path.basename(config.APP_FILE_ICON) + '.icns',
				CFBundleTypeName: 'Winegold Document',
				CFBundleTypeRole: 'Editor',
				LSHandlerRank: 'Owner',
				LSItemContentTypes: ['io.winegold.wgml']
			},
			{
		...
		infoPlist.CFBundleURLTypes = [
			{
				...
				CFBundleURLName: 'Winegold URL',
				CFBundleURLSchemes: ['winegold']
			}
		];
		...
		infoPlist.UTExportedTypeDeclarations = [
			{
				...
				UTTypeTagSpecification: {
					...
					'public.mime-type': 'application/wgml'
				}
			}
		];
    ...
    // Copy file icon into app bundle
      ...
      // Create a .dmg (Mac disk image) file, for easy user installation.
			...
			dmg.once('finish', info => {
				console.log('Mac: Created dmg.  ' + info);
				cb(null);
			});
		}
	});
...
function buildWin32(cb) {
   ...
   */
	...
	electronPackager(Object.assign({}, all, win32), (err, buildPath) => {
         ...
         */
        // To do: Re-enable Windows 64-bit delta updates when we confirm that they
	...
	if (destArch === 'ia32') {
		...
		fs.renameSync(
          ...
          // From winegold-vX.X.X-full.nupkg -> winegold-vX.X.X-ia32-full.nupkg
		fs.renameSync(
